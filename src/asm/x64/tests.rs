...
mod mov {
    ...
    fn test_mov_reg64_ptr64_1() {
        ...
        assert_eq!(code.get_buf(), vec![0x4C, 0x8B, 0x45, 0x00]); // mov r8 [rbp]
    }
    #[test]
    fn test_mov_reg64_ptr64_3() {
        let mut code = EmitterX64::new();
        code.mov_reg64_ptr64(RegX64::R15, RegX64::RSI);
        assert_eq!(code.get_buf(), vec![0x4C, 0x8B, 0x3E]); // mov r15, [rsi]
    }
    #[test]
    fn test_mov_reg64_ptr64_4() {
        let mut code = EmitterX64::new();
        code.mov_reg64_ptr64(RegX64::RDI, RegX64::RBX);
        assert_eq!(code.get_buf(), vec![0x48, 0x8B, 0x3B]); // mov rdi,[rbx]
    }
    #[test]
    fn test_mov_reg64_ptr64_5() {
        let mut code = EmitterX64::new();
        code.mov_reg64_ptr64(RegX64::RAX, RegX64::RAX);
        assert_eq!(code.get_buf(), vec![0x48, 0x8B, 0x00]); // mov rax,[rax]
    }
    #[test]
    fn test_mov_reg64_ptr64_6() {
        let mut code = EmitterX64::new();
        code.mov_reg64_ptr64(RegX64::R11, RegX64::RCX);
        assert_eq!(code.get_buf(), vec![0x4C, 0x8B, 0x19]); // mov r11,[rcx]
    }
    #[test]
    fn test_mov_reg64_ptr64_7() {
        let mut code = EmitterX64::new();
        code.mov_reg64_ptr64(RegX64::RBP, RegX64::RSP);
        assert_eq!(code.get_buf(), vec![0x48, 0x8B, 0x2C, 0x24]); //mov rbp,[rsp]
    }
    #[test]
    fn test_mov_reg64_ptr64_8() {
        let mut code = EmitterX64::new();
        code.mov_reg64_ptr64(RegX64::RCX, RegX64::RDI);
        assert_eq!(code.get_buf(), vec![0x48, 0x8B, 0x0F]); // mov rcx,[rdi]
    }
    #[test]
    fn test_mov_reg64_ptr64_9() {
        let mut code = EmitterX64::new();
        code.mov_reg64_ptr64(RegX64::R9, RegX64::R12);
        assert_eq!(code.get_buf(), vec![0x4D, 0x8B, 0x0C, 0x24]) // mov r9,[r12]
    }
    #[test]
    fn test_mov_reg64_ptr64_10() {
        let mut code = EmitterX64::new();
        code.mov_reg64_ptr64(RegX64::RAX, RegX64::R13);
        assert_eq!(code.get_buf(), vec![0x49, 0x8B, 0x45, 0x00]) // mov rax,[r13]
    }
    ...
    fn test_mov_ptr64_reg64_1() {
        ...
        assert_eq!(code.get_buf(), vec![0x48, 0x89, 0x7D, 0x00]) // mov [rbp],rdi
    }
    #[test]
    fn test_mov_ptr64_reg64_2() {
        let mut code = EmitterX64::new();
        code.mov_ptr64_reg64(RegX64::RSP, RegX64::RAX);
        assert_eq!(code.get_buf(), vec![0x48, 0x89, 0x04, 0x24]) // mov [rsp],rax
    }
    #[test]
    fn test_mov_ptr64_reg64_3() {
        let mut code = EmitterX64::new();
        code.mov_ptr64_reg64(RegX64::R12, RegX64::R15);
        assert_eq!(code.get_buf(), vec![0x4D, 0x89, 0x3C, 0x24]) // mov [r12],r15
    }
    #[test]
    fn test_mov_ptr64_reg64_4() {
        let mut code = EmitterX64::new();
        code.mov_ptr64_reg64(RegX64::R13, RegX64::R13);
        assert_eq!(code.get_buf(), vec![0x4D, 0x89, 0x6D, 0x00]) // mov r13,[r13]
    }
    ...
    fn test_mov_reg64_reg64_1() {
        ...
        assert_eq!(code.get_buf(), vec![0x48, 0x89, 0xD3]) // mov rbx,rdx
    }
    #[test]
    fn test_mov_reg64_reg64_2() {
        let mut code = EmitterX64::new();
        code.mov_reg64_reg64(RegX64::RDX, RegX64::RBP);
        assert_eq!(code.get_buf(), vec![0x48, 0x89, 0xEA]) // mov rdx,rbp
    }
    #[test]
    fn test_mov_reg64_reg64_3() {
        let mut code = EmitterX64::new();
        code.mov_reg64_reg64(RegX64::R9, RegX64::RSP);
        assert_eq!(code.get_buf(), vec![0x49, 0x89, 0xE1]) // mov r9,rsp
    }
    #[test]
    fn test_mov_reg64_reg64_4() {
        let mut code = EmitterX64::new();
        code.mov_reg64_reg64(RegX64::RCX, RegX64::R12);
        assert_eq!(code.get_buf(), vec![0x4C, 0x89, 0xE1]) // mov rcx,r12
    }
